  ...
  const submit = async () => {
	...
	const groupBy = (data: iUserActive[] | undefined): ReportGroup[] => {
		if (data) {
			...
			return Object.entries(groupedKeys).map(([label, data]) => {
				return {
					comment: label,
					data
				};
...
<div class="columns">
  <div class="column is-narrow">
    <div class="control select has-icons-left is-fullwidth">
      <select class="input is-link" bind:value={comment} placeholder="Select comment" on:change={changeGroup}>
      ...
      <div class="columns is-gapless is-mobile">
        <div class="column mr-2">
          <div class="control has-icons-left">
            <input type="text" class="input is-link" bind:value={userName} placeholder="User name" />
            <span class="icon is-small is-left">
